$(window).on('load', function(){
    ...
    // Para elimiar el empleado
    $('.eliminarEmpleado').click(function(e){

        e.preventDefault();

        var empleado = $(this).val(); // value del boton (id de USER)
        var form = $(this).parents('form'); // formulario de este boton, formulario padre
        var url = form.attr('action').replace(':USER_ID', empleado); // atributo action de dicho formulario
...
//        var tbody = $(this).parents('tbody');
        ...
        if(! confirm('¿Esta seguro que desea eliminar el empleado ' + nombre + '?')){
            ...
            .fail(function(data) {
                alert("error --- " + data.nombre);
            })
            .always(function(data) {
                alert("Completatodo pero " + data.nombre);
            });
